...
def test_public_fund_factors_use_pit_materialized_formation_values() -> None:
    fund_portfolio = pd.DataFrame(
        {
            ...
            "fund_count_holding_stock_qoq_change": [-3.0, 8.0],
            "fund_hold_mv_to_float_mv": [0.01, 0.08],
            "fund_hold_mv_to_float_mv_qoq_change": [-0.005, 0.012],
        }
    )
    ...
    assert result["factor_fund_ownership"].tolist() == [0.01, 0.08]
    assert result["factor_fund_ownership_change"].tolist() == [-0.005, 0.012]
