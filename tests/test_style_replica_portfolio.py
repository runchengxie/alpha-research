...
def test_portfolio_config_inherits_shared_signal_settings() -> None:
    signal_config = StyleReplicaConfig(
        a_slots=2,
        b_slots=1,
        theme_quotas={"growth": 2},
    ...
    assert portfolio_config.a_slots == 2
    assert portfolio_config.b_slots == 1
    assert portfolio_config.theme_quotas == {"growth": 2}
    assert portfolio_config.normal_slot_weight == pytest.approx(0.10)
    assert portfolio_config.max_name_weight == pytest.approx(0.20)
...
def test_build_style_replica_positions_requires_a_date_column() -> None:
    signals = pd.DataFrame([{"symbol": "AAA", "score_a": 1.0}])

    with pytest.raises(ValueError, match="signal_date or trade_date"):
        build_style_replica_positions(signals)
