...
def test_rolling_oos_applies_auditable_trade_day_embargo() -> None:
    @dataclass(frozen=True)
    class Summary:
        observations: int = 1

    fitted_as_of: list[pd.Timestamp] = []

    class Ranker:
        training_summary = Summary()

        def fit(self, _frame: pd.DataFrame, *, as_of_date: pd.Timestamp) -> None:
            fitted_as_of.append(as_of_date)
    ...
    evaluation_dates = pd.DatetimeIndex(dates[-2:])
    ...
    assert fitted_as_of == [dates[-8], dates[-7]]
    assert scored["model_fit_as_of_date"].tolist() == fitted_as_of
    assert scored["last_allowed_label_end_date"].tolist() == fitted_as_of
    assert scored["embargo_trade_days"].eq(5).all()
    assert [item["model_fit_as_of_date"] for item in refits] == fitted_as_of
    assert [item["embargo_excluded_trade_dates"] for item in refits] == [
        tuple(dates[-7:-2]),
        tuple(dates[-6:-1]),
    ]
    assert all(item["embargo_trade_days"] == 5 for item in refits)
    with pytest.raises(ValueError, match="non-negative integer"):
        score_rolling_oos(
            ...
            evaluation_dates=evaluation_dates,
            rolling_folds=2,
            embargo_trade_days=True,  # type: ignore[arg-type]
        )
