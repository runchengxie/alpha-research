...
def test_build_operating_quality_persistence_targets_is_future_and_auditable() -> None:
    frame = pd.DataFrame(
        {
            ...
            "report_period": pd.to_datetime(
                ["2021-12-31", "2022-12-31", "2023-12-31"] * 2
            ),
            "available_date": pd.to_datetime(
                ["2022-03-01", "2023-03-01", "2024-03-01"] * 2
            ),
    ...
    row_a = result.loc[result["symbol"].eq("A") & result["report_period"].eq(pd.Timestamp("2021-12-31"))].iloc[0]
    row_b = result.loc[result["symbol"].eq("B") & result["report_period"].eq(pd.Timestamp("2022-12-31"))].iloc[0]
